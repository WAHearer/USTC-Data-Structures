...
private:
    ...
    void pushDown(HeapNode *now){
        if(now->left&&!now->right){
            ...
            HeapNode *&temp=now->left->val<now->right->val?now->left:now->right;
            if(now->val>temp->val){
                std::swap(now->val,temp->val);
                pushDown(temp);
            }
        }
    }
    ...
    Complete del(int closetime){
        ...
        while(eventlist[p].next!=-1){
            ...
            if(eventlist[temp].leavetime!=-1){
                ...
                solved++;
                sum+=(closetime-eventlist[temp].arrtime);
                p=eventlist[p].next;
            }
        }
        return {solved,sum};
...
public:
    ...
    float imitate(int days){
        ...
        for(int i=1;i<=days;i++){
            generate();
            run();
            Complete result=list.del(closetime);
